  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800/60">
      ...
      {chips.map((chip) => (
        <span
          key={chip.key}
          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-primary/15 text-primary-300 border border-primary/30 shadow-sm"
        >
          <span>{chip.label}</span>
          <button
            type="button"
            onClick={() => onRemoveFilter(chip.key)}
            className="w-3.5 h-3.5 rounded-full hover:bg-primary/30 flex items-center justify-center transition-colors"
            title="Remove filter"
          >
            ×
          </button>
      ...
      <button
        type="button"
        onClick={onResetAll}
        className="text-xs text-rose-400 hover:text-rose-300 font-semibold px-2 py-0.5 rounded-md hover:bg-rose-500/10 transition-colors ml-auto"
      >
        Clear All
      </button>
    </div>
