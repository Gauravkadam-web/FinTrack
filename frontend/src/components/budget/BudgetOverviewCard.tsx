...
export function BudgetOverviewCard({
  ...
  if (!snapshot) {
    return (
      <div className="glass-card rounded-2xl p-4 sm:p-5 flex flex-col justify-between border border-dashed border-slate-300 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-700 transition-colors group h-full">
        <div className="flex items-center justify-between">
          <span className="text-[11px] uppercase tracking-wider font-bold text-slate-500 dark:text-slate-400">
            Monthly Budget
          </span>
          <div className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-surface-100 flex items-center justify-center text-slate-400 border border-slate-200 dark:border-slate-800">
    ...
    <div className="glass-card rounded-2xl p-4 sm:p-5 flex flex-col justify-between border border-slate-200/80 dark:border-slate-800/80 hover:border-slate-300 dark:hover:border-slate-700/80 transition-all h-full">
      ...
      <div className="flex items-center justify-between gap-1.5 mb-1.5">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            onClick={onSetBudget}
            className="p-1 rounded-md text-slate-400 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-surface-100 transition-colors"
      ...
      <div className="my-1">
        <div
          className={`text-xl sm:text-2xl font-extrabold tracking-tight truncate ${
            remainingAmount < 0 ? "text-rose-600 dark:text-rose-400" : "text-slate-900 dark:text-white"
          }`}
        >
      ...
      <div className="space-y-1 pt-1">
        ...
        <div className="w-full h-1.5 bg-slate-200 dark:bg-surface-200 rounded-full overflow-hidden">
          <div
