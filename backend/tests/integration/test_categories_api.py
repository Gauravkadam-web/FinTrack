...
@pytest.mark.asyncio
async def test_categories_api_crud_flow(client):
    # 1. List categories (seeded categories should be present)
    ...
    patch_res = await client.patch(
        f"/api/v1/categories/{cat_id}", json={"name": renamed}
    )
    assert patch_res.status_code == 200
    assert patch_res.json()["name"] == renamed

    # 4. Check expense count
    count_res = await client.get(f"/api/v1/categories/{cat_id}/expense-count")
    assert count_res.status_code == 200
    assert count_res.json()["expense_count"] == 0

    # 5. Delete category
    delete_res = await client.delete(f"/api/v1/categories/{cat_id}")
    assert delete_res.status_code == 204


@pytest.mark.asyncio
async def test_categories_validation_and_errors(client):
    # Empty name validation error
    res = await client.post("/api/v1/categories", json={"name": "   "})
    ...
    del_res = await client.delete(f"/api/v1/categories/{uncat['id']}")
