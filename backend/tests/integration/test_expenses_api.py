...
@pytest.mark.asyncio
async def test_expenses_api_crud_flow(client):
    # Get a category
    ...
    create_res = await client.post("/api/v1/expenses", json=payload)
    ...
    put_res = await client.put(f"/api/v1/expenses/{expense_id}", json=update_payload)
    assert put_res.status_code == 200
    assert put_res.json()["title"] == "Dinner with team (Updated)"
    assert put_res.json()["payment_mode"] == "card"

    # 4. Search and pagination
    list_res = await client.get("/api/v1/expenses?search=celebration")
    assert list_res.status_code == 200

    list_res_title = await client.get("/api/v1/expenses?search=Dinner")
    ...
    # Verify not found
    get_after_del = await client.get(f"/api/v1/expenses/{expense_id}")
    assert get_after_del.status_code == 404


@pytest.mark.asyncio
async def test_expense_validations(client):
    cats_res = await client.get("/api/v1/categories")
